...
public class Code {
	...
	public List<Code> getCodesByUser_id(String userId) {
		...
		if (result.size() > 0) {
			for (Code nextCode : result) {
				logger.info(
						"Codes - getCodesByUser_id() for User[ " + userId + " ]" + " Code - " + nextCode.toString());
			}
		} else {
	...
	public Code getCodeByPrize_code(String prizeCode) {
		...
		DynamoDBQueryExpression<Code> queryExp = new DynamoDBQueryExpression<Code>().withIndexName("prize_code-index")
				.withConsistentRead(false).withKeyConditionExpression("prize_code = :v1")
				.withExpressionAttributeValues(av);
